...
for estimationLevel = 1:3%
    ...
    for i = 1:101
        cap_ROW = effe_ratio*Ratio(estimationLevel)*1.98*0.028317/1000*rem_ROW_matrix(:,i);%单位Gt CO2
        cap_US = effe_ratio*Ratio(estimationLevel)*1.98*0.028317/1000*rem_US_matrix(:,i);%单位Gt CO2
    ...
    for i = 1:81
        cap_US = effe_ratio*Ratio(estimationLevel)*1.98*0.028317/1000*US_produced_list(:,i);%单位Gt CO2
        storage_US_Dpl_Gas_list(:,i) = cap_US;
        cap_ROW = effe_ratio*Ratio(estimationLevel)*1.98*0.028317/1000*ROW_produced_list(:,i);%单位Gt CO2
